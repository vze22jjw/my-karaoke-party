...
1.  **👑 Host (Desktop):** * Starts a new party with a unique name (e.g., "User Party 123").
...
2.  **📱 Guest (Mobile):** * Joins via the party code generated by the Host.
...
* 📄 **`report/index.html`**: The full HTML test report.
* 🎥 **`videos/`**: Continuous screen recordings of the Host, Guest, and Player sessions.
* 📸 **`screenshots/`**: PNG snapshots taken at key moments (e.g., `01-player-connected.png`).
