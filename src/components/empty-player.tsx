  ...
  return (
    <div className="flex w-full flex-col items-center justify-center text-center animate-in fade-in-0 duration-1000">
      <div key={currentIndex} className="animate-in fade-in slide-in-from-bottom-8 duration-700">
        <blockquote className="text-outline scroll-m-20 text-4xl font-extrabold tracking-tight text-white lg:text-6xl italic leading-tight drop-shadow-xl max-w-4xl">
          &ldquo;{quote}&rdquo;
        </blockquote>
        {author && (
          <cite className="text-outline mt-8 block text-xl font-semibold tracking-widest text-primary lg:text-3xl not-italic uppercase">
            — {author}
          </cite>
        )}
      </div>
...
export function EmptyPlayer({ joinPartyUrl, className, messages }: Props) {
  ...
  return (
    <div className={cn("relative flex h-full w-full flex-col items-center justify-center p-6 overflow-hidden transition-colors duration-1000 bg-gradient", className)}>
