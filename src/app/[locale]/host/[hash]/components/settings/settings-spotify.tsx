...
export function SettingsSpotify({ partyHash, spotifyPlaylistId, spotifyLink, isPartyClosed }: Props) {
  const t = useTranslations('host.settings.spotify');
  ...
  return (
    <div className="space-y-4 rounded-lg border bg-card p-4">
      ...
      <div className="space-y-2">
        <Label htmlFor="spotify-id">{t('playlistId')}</Label>
        <Input
            id="spotify-id"
            value={spotifyIdInput}
            onChange={(e) => setSpotifyIdInput(e.target.value)}
            placeholder="e.g. 37i9dQZF1DXbITwg1ZjkYt"
            disabled={isPartyClosed}
        />
      </div>

      <div className="space-y-2">
        <Label htmlFor="spotify-link">{t('link')}</Label>
