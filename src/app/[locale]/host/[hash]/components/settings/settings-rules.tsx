    ...
    <div className="space-y-3 rounded-lg border bg-card p-3">
      ...
      {showRulesInfo && (
        <Alert className="mt-2 space-y-2">
          <AlertDescription>{t('infoQueue')}</AlertDescription>
          <AlertDescription>{t('infoSort')}</AlertDescription>
          <AlertDescription>{t('infoPlayback')}</AlertDescription>
        </Alert>
      )}
      ...
      <ToggleButton
        ...
        label={`${t('manualSort')}: ${isManualSortActive ? "ON" : "OFF"}`}
      ...
      <ToggleButton
        ...
        label={useQueueRules ? t('fairnessOn') : t('fairnessOff')}
        disabled={isPartyClosed}
      />
      ...
      <ToggleButton
        ...
        label={!disablePlayback ? t('playbackApp') : t('playbackYoutube')}
        disabled={isPartyClosed}
      />
    </div>
  );
